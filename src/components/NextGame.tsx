...
export default function NextGame({ schedule }: NextGameProps) {
  ...
  if (!upcomingGame) {
    return <div className="p-4 bg-mist-700 rounded-lg text-center">No more games this season. See you next year!</div>;
  }

  return (
    <div className="p-4 bg-mist-700 rounded-lg">
        <h2 className="text-xl font-bold mb-2">Next Game</h2>
        <p>{format(upcomingGame.dateObj, 'EEEE, MMMM d, yyyy')}</p>
        <p>{format(upcomingGame.dateObj, 'p')} on Court {upcomingGame.court}</p>
